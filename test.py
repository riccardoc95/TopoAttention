...
def save_array_as_image(array, filename):
    # Check if array is already in uint8 format
    if array.dtype != np.uint8:
        # Normalize to 0–255 and convert to uint8
        array = (255 * (array - np.min(array)) / (np.max(array) - np.min(array))).astype(np.uint8)

    # Handle grayscale (2D array) and RGB (3D array) automatically
...
group.add_argument('--topo', action='store_true', default=False,
                   help='Add topological information to data')
group.add_argument('--denoising', action='store_true', default=False,
                   help='Set to denoising task')
group.add_argument('--noise-level', type=float, default=0.01,
                   help='Noise Level')
group.add_argument('--segmentation', action='store_true', default=False,
                   help='Set to segmentation task')
group.add_argument('--segmentation-level', type=float, default=0.5,
                   help='Segment Generic Dataset')
...
group.add_argument('--model-img-size', default=None, nargs=2, type=int,
                   metavar='N N',
...
group.add_argument('--output-img', default='', type=str, metavar='PATH',
                   help='path to output image folder (default: none, current dir)')
...
def main():
    ...
    if args.model_img_size:
        args.model_kwargs["img_size"] = args.model_img_size[0]
    model = create_model(
        args.model,
        ...
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                if isinstance(output, (tuple, list)):
                    output = output[0]
                if segm:
                    output = (torch.sigmoid(output) > 0.5) * 255.0

                if output_dir is not None:
                    if "restored" in output_dir:
                        general_output_dir = output_dir[:output_dir.find("restored")]
                    else:
                        general_output_dir = output_dir[:output_dir.find("segm")]
                    for n in range(output.shape[0]):
                        img = output[n].detach().squeeze().cpu().numpy()
                        if not segm:
                            img = np.clip(img, -1, 1)
                            img = (img - img.min()) / (img.max() - img.min())


                        output_name = os.path.join(output_dir, f"{n_img}.{extension}")

                        if save_all:
                            img_input = input[n].detach().squeeze().cpu().numpy()
                            img_target = target[n].detach().squeeze().cpu().numpy()
                            output_name_input = os.path.join(general_output_dir, "input", f"{n_img}.{extension}")
                            output_name_target = os.path.join(general_output_dir, "target", f"{n_img}.{extension}")
                        if extension == "fits":
                            fits.writeto(output_name, img, overwrite=True)
                            if save_all:
                                fits.writeto(output_name_input, img_input, overwrite=True)
            ...
            if args.denoising or args.segmentation:
                acc1 = torch.tensor(0.0)
                acc5 = torch.tensor(0.0)
            else:
