...
def fast_collate(batch):
    ...
    if isinstance(batch[0][0], tuple):
        ...
        for i in range(batch_size):
            tensor[i] += torch.from_numpy(batch[i][0]).to(torch.uint8)
        return tensor, targets
    ...
    def __iter__(self):
        ...
        for next_input, next_target in self.loader:
            ...
            with stream_context():
                ...
                if self.topo:
                    b, c, h, w = next_input.shape
                    next_topo = next_input[:, 0, :, :].unsqueeze(1)
                    next_topo = torch.nn.functional.normalize(next_topo,dim=1)
                ...
                if self.topo:
                    next_input = torch.cat((next_topo, next_input), dim=1)
...
def create_loader(
    ...
    if collate_fn is None:
        collate_fn = torch.utils.data.dataloader.default_collate#fast_collate if use_prefetcher else torch.utils.data.dataloader.default_collate
