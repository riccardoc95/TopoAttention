...
from .dataset import IterableImageDataset, ImageDataset, DenoisingDataset, SegmentationDataset
from .illustris import ILLUSTRIS
from .illustris_segm import ILLUSTRIS as IllustrisSegmentation
from .nasa_apd_galaxysegment import GalaxySegmentationDataset
...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        if name in _TORCH_BASIC_DS:
            ...
            if name in ["voc_segmentation", "voc_detection"]:
                ds = ds_class(image_set="train" if use_train else "val", **torch_kwargs)
            else:
        ...
        elif name == "nasa-apd-galaxysegment":
            segmentation_ds = True
            ds = GalaxySegmentationDataset(
                folder=root,
                dim=128,
                is_training=split == 'train',
            )
        elif name == "illustris-segment":
            segmentation_ds = True
            ds = IllustrisSegmentation(
                folder=root,
                dim=128,
                is_training=split == 'train',
            )
    ...
    if denoising and not denoising_ds:
        ds = DenoisingDataset(ds, noise_level=noise_level)
    if segmentation and not segmentation_ds:
        ds = SegmentationDataset(ds, segmentation_level=segmentation_level)
    return ds
