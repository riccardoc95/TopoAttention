...
class DenseNet(nn.Module):
    ...
    def __init__(
        ...
        if deep_stem:
            ...
            self.features = nn.Sequential(OrderedDict([
                ('conv0', nn.Conv2d(in_chans, num_init_features, kernel_size=3, stride=1, padding=1, bias=False)),
...
@register_model
def densenet121small(pretrained=False, **kwargs) -> DenseNet:
    r"""Densenet-121 model from
    `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`
    """
    model_args = dict(growth_rate=32, block_config=(6, 12, 24, 16), stem_type='small')
